...
export const JAVASCRIPT_URL_XSS = /[u00-u1F]*j[\s]*a[\s]*v[\s]*a[\s]*s[\s]*c[\s]*r[\s]*i[\s]*p[\s]*t[\s]*:/i
export const QUERY_SPLIT_REGEX = /[\s.,]+/gm
export const SPECIAL_CHAR_REGEXP = /[^\p{L}\s-]/ui
export const LINEAR_GRADIENT_REGEXP = /^linear-gradient\([^(]*(\([^)]*\)[^(]*)*[^)]*\)$/
export const HEX_CODE_REGEXP = /^#(([0-9a-f]){3}){1,2}$/i
export const TWO_OR_MORE_SPACES_REGEXP = /\s\s+/g
export const OMIT_SEARCH_CHARACTERS_REGEXP = /[^\p{Alphabetic}\p{Decimal_Number}\s/]/igu
