...
const OrganizationProvider = ({ children, initialLinkValue }) => {
  const auth = useAuth();
  const [linkIdState, setLinkIdState] = useState(
    (initialLinkValue && initialLinkValue.id) || getLinkId()
  );
  const [link, setLink] = useState(initialLinkValue);

  const { loading: linkLoading, refetch } = useQuery(
    GET_ORGANIZATION_TO_USER_LINK_BY_ID_QUERY,
    {
      ...
      onCompleted: (data) => {
        ...
        if (DEBUG_RERENDERS)
          console.log("OrganizationProvider() newState", obj);
  ...
  if (DEBUG_RERENDERS)
    console.log(
      ...
      linkLoading,
      "skip",
      auth.isLoading || !auth.user || !linkIdState
    );
...
if (DEBUG_RERENDERS_BY_WHY_DID_YOU_RENDER)
  OrganizationProvider.whyDidYouRender = true;
...
const withOrganization = ({ ssr = false, ...opts } = {}) => (PageComponent) => {
  ...
  GET_ORGANIZATION_TO_USER_LINK_BY_ID_QUERY = opts.GET_ORGANIZATION_TO_USER_LINK_BY_ID_QUERY
    ? opts.GET_ORGANIZATION_TO_USER_LINK_BY_ID_QUERY
    : GET_ORGANIZATION_TO_USER_LINK_BY_ID_QUERY;
  ...
  if (DEBUG_RERENDERS_BY_WHY_DID_YOU_RENDER)
    WithOrganization.whyDidYouRender = true;
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithOrganization.getInitialProps = async (ctx) => {
      if (DEBUG_RERENDERS)
        console.log("WithOrganization.getInitialProps()", ctx);
      const isOnServerSide = typeof window === "undefined";
      const { link } = await initOnRestore(ctx);
      const pageProps = await getContextIndependentWrappedInitialProps(
        PageComponent,
        ctx
      );
...
export { withOrganization, useOrganization };
