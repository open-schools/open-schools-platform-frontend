...
export default withApollo({ ssr: true })(
  withIntl({
    ssr: true,
    messagesImporter: (locale) => import(`../lang/${locale}`),
  })(withAuth({ ssr: false })(MyApp))
);
```
...
const SignInPage = () => {
  ...
  const clickHandler = () => {
    signin({
      variables: { email: "admin@example.com", password: "secret" },
    }).then(
      () => {
...
const RecoveryPage = () => {
  ...
  const clickHandler = () => {
    startPasswordRecovery({ variables: { email: "admin@example.com" } }).then(
      () => {
        console.log("SIGNED_IN");
      },
      (e) => {
