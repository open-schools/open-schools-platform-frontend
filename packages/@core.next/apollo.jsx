...
let getApolloClientConfig = () => {
  ...
  if (!serverUrl || !apolloGraphQLUrl)
    throw new Error(
      "You should set next.js publicRuntimeConfig { serverUrl, apolloGraphQLUrl } variables. Check your next.config.js"
    );
...
let createApolloClient = (initialState, ctx) => {
  ...
  if (DEBUG_RERENDERS)
    console.log("WithApollo(): getApolloClientConfig()", {
      serverUrl,
      apolloGraphQLUrl,
    });
  ...
  if (isOnClientSide && window.location && window.location.href) {
    ...
    if (!window.location.href.startsWith(serverUrl)) {
      ...
      console.warn(
        `Your serverUrl=${serverUrl}! Your window.location have another domain! `
      );
    }
  }
...
const withApollo = ({ ssr = false, ...opts } = {}) => (PageComponent) => {
  ...
  getApolloClientConfig = opts.getApolloClientConfig
    ? opts.getApolloClientConfig
    : getApolloClientConfig;
  createApolloClient = opts.createApolloClient
    ? opts.createApolloClient
    : createApolloClient;
