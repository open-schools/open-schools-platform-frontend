...
function genReactHooks(
  TestUtils,
  { convertGQLItemToUIState, convertUIStateToGQLItem, options } = {
    options: {},
  }
) {
  function useObject(variables) {
    ...
    if (count && count > 1)
      throw new Error("Wrong query condition! return more then one result");
    const obj = objs.length ? objs[0] : null;
    return { loading, refetch, obj, error };
  }

  function useObjects(variables = {}) {
    let { loading, data, refetch, error } = useQuery(
      TestUtils.GET_ALL_OBJS_WITH_COUNT_QUERY,
      {
        variables,
        ...options,
      }
    ...
    const intl = useIntl();
    const ServerErrorPleaseTryAgainLaterMsg = intl.formatMessage({
      id: "ServerErrorPleaseTryAgainLater",
    });
    const AccessErrorMsg = intl.formatMessage({ id: "AccessError" });

    const objs = useMemo(
      () => (data && data.objs ? data.objs.map(convertGQLItemToUIState) : []),
      [data]
    );
  ...
  function useCreate(attrs = {}, onComplete) {
    if (typeof attrs !== "object" || !attrs)
      throw new Error("useCreate(): invalid attrs argument");
  ...
  function useUpdate(attrs = {}, onComplete) {
    if (typeof attrs !== "object" || !attrs)
      throw new Error("useUpdate(): invalid attrs argument");
