    ...
    if (GQL_SCHEMA_TYPES.includes(module._type)) {
      ...
      Object.values(module).forEach((GQLSchema) => {
        if (GQL_SCHEMA_TYPES.includes(GQLSchema._type)) {
          ...
          console.warn(
            "Wrong schema module export format! What's this? ",
            GQLSchema
          );
        }
      });
...
async function unregisterAllSchemas() {
  console.warn(
    "unregisterAllSchemas() called! It'just for debug/tests purposes. Don't use it in prod!"
  );
  EVENTS = new Emittery();
  SCHEMAS = new Map();
}
...
class GQLListSchema {
  constructor(name, schema) {
    ow(
      ...
      ow.object.partialShape({
        fields: ow.object.valuesOfType(
          ow.any(ow.object.hasKeys("type"), ow.null, ow.undefined)
        ),
  ...
  _override(schema) {
    ...
    Object.keys(schema).forEach((key) => {
      if (
        isNotNullObject(schema[key]) &&
        isNotNullObject(this.schema[key]) &&
  ...
  _register(keystone) {
    if (SCHEMAS.has(this.name))
      throw new Error(`Schema ${this.name} is already registered`);
      ...
      ow.object.partialShape({
        mutations: ow.array.valueOf(
          ow.object.valuesOfType(
            ow.object.hasKeys(["access", "schema", "resolver"])
          )
        ),
      })
    );
    if (!name.endsWith("Service"))
      console.warn(`GQLCustomSchema name=${name} is not ends with 'Service'`);
    this.name = name;
    this.schema = schema;
...
async function find(schemaName, condition) {
  if (!SCHEMAS.has(schemaName))
    throw new Error(`Schema ${schemaName} is not registered yet`);
...
async function getByCondition(schemaName, condition) {
  ...
  if (res.length > 1)
    throw new Error("getByCondition() returns multiple objects");
...
async function getSchemaCtx(schemaObjOrName) {
  let name;
  if (
    typeof schemaObjOrName === "object" &&
    GQL_SCHEMA_TYPES.includes(schemaObjOrName._type) &&
    schemaObjOrName.name
  ) {
