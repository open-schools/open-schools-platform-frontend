...
function prepareHistoryRecordFields(
  listKey,
  listFields,
  historyField,
  ignoreFieldTypes
  ...
  const historyFields = Object.fromEntries(
    Object.entries(listFields)
      ...
      .map(([k, v]) => {
        const newValue = (typeMapping[v.type.type]
          ? typeMapping[v.type.type]
          : defaultMapping)(v);
        if (v.access) {
...
const historical = ({
  ...
  ignoreFieldTypes = ["Content"],
  isStrictMode = true,
} = {}) => (
  { fields = {}, hooks = {}, access, ...rest },
  { listKey, keystone }
) => {
  const historicalFields = prepareHistoryRecordFields(
    listKey,
    fields,
    historyField,
    ignoreFieldTypes
  ...
  const historicalList = createHistoricalList(
    keystone,
    access,
    historicalListKey,
    historicalFields
  ...
  const hook = async (hookArgs) => {
    const hist = prepareHistoryRecordInput(
      hookArgs,
      historyField,
      historicalFields,
      historicalList
