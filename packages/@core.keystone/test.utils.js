...
function setFakeClientMode(path) {
  ...
  let mode = null;
  if (
    module.hasOwnProperty("URL_PREFIX") &&
    module.hasOwnProperty("prepareBackApp")
  ) {
    mode = "multi-server";
    ...
    module.hasOwnProperty("keystone") &&
    module.hasOwnProperty("apps")
  ) {
    mode = "keystone";
    beforeAll(async (done) => {
  ...
  return {
    mutate: async (query, variables = {}) => {
      if (query.kind !== "Document")
        throw new Error("query is not a gql object");
...
const makeLoggedInAdminClient = async () => {
  return await makeLoggedInClient({
...
const createUser = async (args = {}) => {
  ...
  if (result.errors && result.errors.length > 0) {
    console.warn(
      util.inspect(result.errors, { showHidden: false, depth: null })
    );
...
const createSchemaObject = async (schemaList, args = {}) => {
  if (schemaList._type !== GQL_LIST_SCHEMA_TYPE)
    throw new Error(`Wrong type. Expect ${GQL_LIST_SCHEMA_TYPE}`);
  const client = await makeLoggedInAdminClient();
...
const getSchemaObject = async (schemaList, fields, where) => {
  ...
  function fieldsToStr(fields) {
    return (
      "{ " +
      fields.map((f) => (Array.isArray(f) ? fieldsToStr(f) : f)).join(" ") +
      " }"
    );
  }
