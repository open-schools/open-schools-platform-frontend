...
const RESET_PASSWORD_TOKEN_EXPIRY =
  conf.USER__RESET_PASSWORD_TOKEN_EXPIRY || 1000 * 60 * 60 * 24;
  ...
  mutations: [
    {
      ...
      resolver: async (parent, args, context, info, extra = {}) => {
        ...
        const extraTokenExpiration =
          extra.extraTokenExpiration || parseInt(RESET_PASSWORD_TOKEN_EXPIRY);
        const extraNowTimestamp = extra.extraNowTimestamp || Date.now();

        const requestedAt = new Date(extraNowTimestamp).toISOString();
        const expiresAt = new Date(
          extraNowTimestamp + extraTokenExpiration
        ).toISOString();
        ...
        if (userErrors || !userData.allUsers || !userData.allUsers.length) {
          throw new Error(
            "[unknown-user]: Unable to find user when trying to start password recovery"
          );
        }

        if (userData.allUsers.length !== 1) {
          throw new Error(
            "[unknown-user]: Unable to find exact one user to start password recovery"
          );
        }
        ...
        if (createErrors) {
          ...
          throw new Error(
            "[error]: Unable to create forgotten password action"
          );
        }

        // prepare emit context
        const {
          errors: userAndTokenErrors,
          data,
        } = await context.executeGraphQL({
          context: context.createContext({ skipAccessControl: true }),
          query: `
        ...
        if (userAndTokenErrors) {
          ...
          throw new Error(
            "[error]: Unable to construct forgot password context"
          );
        }
    ...
    },
    {
      access: true,
      schema:
        "changePasswordWithToken(token: String!, password: String!): String",
      resolver: async (parent, args, context, info, extra) => {
  ...
  types: [
    {
      access: true,
      type:
        "input RegisterNewUserInput { name: String!, email: String!, password: String! }",
    },
  ],
  mutations: [
    {
      ...
      resolver: async (parent, args, context, info, extra = {}) => {
...

        {
          ...
          if (data.users.length !== 0) {
            throw new Error(
              "[register:email:multipleFound] User with this email is already registered"
            );
          }
        }
        ...
        if (password.length < 8) {
          throw new Error(
            "[register:password:minLength] Password length less then 7 character"
          );
        }
