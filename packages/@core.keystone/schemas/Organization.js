...
const OrganizationToUserLink = new GQLListSchema("OrganizationToUserLink", {
  fields: {
    ...
    name: {
      factory: () =>
        faker.fake("{{name.suffix}} {{name.firstName}} {{name.lastName}}"),
      type: Text,
    },
    user: {
...
const RegisterNewOrganizationService = new GQLCustomSchema(
    ...
    mutations: [
      {
        ...
        resolver: async (parent, args, context, info, extra = {}) => {
          await RegisterNewOrganizationService.emit(
            "beforeRegisterNewOrganization",
            {
              parent,
              args,
              context,
          ...
          const result = await getById(
            "Organization",
            createData.obj.organization.id
          );
          await RegisterNewOrganizationService.emit(
            "afterRegisterNewOrganization",
            {
              parent,
              args,
              context,
...
const InviteNewUserToOrganizationService = new GQLCustomSchema(
    ...
    types: [
      {
        access: true,
        type:
          "input InviteNewUserToOrganizationInput { organization: OrganizationWhereUniqueInput!, email: String!, name: String }",
      },
    ],
    mutations: [
      {
        ...
        schema:
          "inviteNewUserToOrganization(data: InviteNewUserToOrganizationInput!): OrganizationToUserLink",
        resolver: async (parent, args, context, info, extra = {}) => {
          await InviteNewUserToOrganizationService.emit(
            "beforeInviteNewUserToOrganization",
            {
              parent,
              args,
              context,
          ...
          // Note: check is already exists (email + organization)
          {
            ...
            if (data.objs.length > 0) {
              const msg =
                "[error.already.exists] User is already invited in the organization";
              console.error(msg, errors);
              throw new Error(msg);
            }
          }
          ...
          await InviteNewUserToOrganizationService.emit(
            "afterInviteNewUserToOrganization",
            {
              parent,
              args,
              context,
...
const AcceptOrRejectOrganizationInviteService = new GQLCustomSchema(
    ...
    types: [
      {
        access: true,
        type:
          "input AcceptOrRejectOrganizationInviteInput { isRejected: Boolean, isAccepted: Boolean }",
      },
    ],
    mutations: [
      {
        ...
        schema:
          "acceptOrRejectOrganizationInviteByCode(code: String!, data: AcceptOrRejectOrganizationInviteInput!): OrganizationToUserLink",
        resolver: async (parent, args, context, info, extra = {}) => {
          await AcceptOrRejectOrganizationInviteService.emit(
            "beforeAcceptOrRejectOrganizationInviteInput",
            {
              parent,
              args,
              context,
          ...
          const link = await getByCondition("OrganizationToUserLink", {
            code,
            user_is_null: true,
          });

          const {
            errors: acceptOrRejectErrors,
            data: acceptOrRejectData,
          } = await context.executeGraphQL({
            context: context.createContext({ skipAccessControl: true }),
            query: `
          ...
          if (
            acceptOrRejectErrors ||
            !acceptOrRejectData.obj ||
            !acceptOrRejectData.obj.id
          ) {
          ...
          await AcceptOrRejectOrganizationInviteService.emit(
            "afterAcceptOrRejectOrganizationInviteInput",
            {
              parent,
              args,
              context,
      ...
      },
      {
        ...
        schema:
          "acceptOrRejectOrganizationInviteById(id: ID!, data: AcceptOrRejectOrganizationInviteInput!): OrganizationToUserLink",
        resolver: async (parent, args, context, info, extra = {}) => {
          await AcceptOrRejectOrganizationInviteService.emit(
            "beforeAcceptOrRejectOrganizationInviteInput",
            {
...
async function accessAllowOnlyForRoleOwner({
  operation,
  ...
  if (operation === "create" && originalInput) {
    if (
      !connectByIdOnly(originalInput.organization) ||
      !connectByIdOnly(originalInput.user)
    )
      return false;
...
async function allowAccessForRoleOwnerForInviteNewUserToOrganizationService({
  authentication: { item: user },
  args,
  context,
}) {
