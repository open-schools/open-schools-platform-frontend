...
class OptionsImplementation extends Implementation {
  ...
  `
    ...
    const { itemQueryName } = listConfig.getListByKey(
      listConfig.listKey
    ).gqlNames;
  ...
  gqlOutputFieldResolvers() {
    return {
      [this.path]: (item) => {
        const default_ = this.options.reduce(
  ...
  getDefaultValue({ context, originalInput, actions }) {
    ...
    if (typeof this.defaultValue !== "undefined") {
      if (typeof this.defaultValue === "function") {
        Object.assign(
          default_,
          this.defaultValue({ context, originalInput, actions })
        );
      } else {
  ...
  async resolveInput({ resolvedData, existingItem }) {
    const defaultData = this.options.reduce(
      (prev, next) => ({ ...prev, [next]: null }),
      {}
    );
    ...
    const nonNullKeys = Object.keys(mergedData).filter(
      (k) => mergedData[k] !== null
    );
