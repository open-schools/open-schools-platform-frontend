...
class AutoIncrementInteger extends Integer.implementation {
  async validateInput({ resolvedData, addFieldValidationError }) {
    ...
    if (value) {
      ...
      const { count } = await listAdapter.itemsQuery(
        { where: { [this.path]: value } },
        { meta: true }
      );
      if (count) {
        addFieldValidationError(
          `[unique:alreadyExists:${this.path}] Field ${this.path} should be unique`
        );
      }
    }
  }
  ...
  setupHooks({ addPreSaveHook, addPostReadHook }) {
    addPreSaveHook((item) => {
      ...
      item[
        ...
        .raw(
          "coalesce( (select max(:fieldName:) as max from :tableName:), 0) + 1",
          { tableName, fieldName }
        )
        .wrap("(", ")");
      return item;
...
class AutoIncrementIntegerMongooseFieldAdapter extends Integer.adapters
  .mongoose {
  setupHooks({ addPreSaveHook, addPostReadHook }) {
    addPreSaveHook((item) => {
