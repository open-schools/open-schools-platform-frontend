...
function getCookieSecret(cookieSecret) {
  if (!cookieSecret) {
    if (process.env.NODE_ENV === "production") {
      throw new TypeError(
        "getCookieSecret() call without cookieSecret (check the COOKIE_SECRET environment)"
      );
    }
    return undefined;
  }
  if (typeof cookieSecret !== "string")
    throw new TypeError("getCookieSecret() cookieSecret is not a string");
...
function getAdapter(databaseUrl) {
  if (!databaseUrl)
    throw new TypeError("getAdapter() call without databaseUrl");
  if (typeof databaseUrl !== "string")
    throw new TypeError("getAdapter() databaseUrl is not a string");
