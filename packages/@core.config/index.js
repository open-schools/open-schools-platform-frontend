...
if (fs.existsSync(path.join(root, ".env"))) {
  ...
  for (const k in env1) {
    if (!env0.hasOwnProperty(k)) {
      if (DEBUG)
        console.log(`@core.config: [root.env] process.env[${k}] = ${env1[k]}`);
...
if (root !== cwd) {
  ...
  if (fs.existsSync(path.join(cwd, ".env"))) {
    ...
    for (const k in env2) {
      if (!env0.hasOwnProperty(k)) {
        if (DEBUG)
          console.log(`@core.config: [app.env] process.env[${k}] = ${env2[k]}`);
...
if (DEBUG)
  console.log(
    `@core.config: inited! namespace=${namespace}, cwd=${cwd}, root=${root}`
  );
if (DEBUG) console.dir(process.env);

function getEnv(namespace, name, defaultValue) {
  return preprocessEnv(
    process.env[`${namespace}_${name}`] ||
      process.env[`${name}`] ||
      defaultValue
  );
}
